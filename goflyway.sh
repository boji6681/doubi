...
Service_goflyway(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/boji6681/doubi/master/service/goflyway_centos -O /etc/init.d/goflyway; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/boji6681/doubi/master/service/goflyway_debian -O /etc/init.d/goflyway; then
			echo -e "${Error} GoFlyway 服务管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/goflyway
...
Set_passwd(){
	...
	read -e -p "(默认: Tg@bojiiking):" new_passwd
	[[ -z "${new_passwd}" ]] && new_passwd="Tg@bojiiking"
	echo && echo "========================"
...
link_qr(){
	...
	link=" 链接\t: ${Red_font_prefix}${url}${Font_color_suffix} \n 二维码已失效 : ${Red_font_prefix}${QRcode}${Font_color_suffix} \n "
}
View_Log(){
	check_installed_status
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/boji6681/doubi/master/goflyway.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/goflyway" ]]; then
		rm -rf /etc/init.d/goflyway
		Service_goflyway
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/boji6681/doubi/master/goflyway.sh" && chmod +x goflyway.sh
...
echo && echo -e "┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
┃  GoFlyway 一键管理脚本 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}　 ┃
┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
┃   机场搭建 ❤ 机场托管 ❤ 技术支持 　┃ 
┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
