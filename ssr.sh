...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/boji6681/doubi/master/bbr.sh; then
...
Set_config_port(){
	...
	read -e -p "(默认: 23456):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="23456"
...
Set_config_password(){
	...
	read -e -p "(默认: Tg@bojiiking):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="Tg@bojiiking"
...
Download_SSR(){
	cd "/usr/local/"
	wget -N --no-check-certificate "https://github.com/boji6681/shadowsocksr/archive/manyuser.zip"
	#git config --global http.sslVerify false
	#env GIT_SSL_NO_VERIFY=true git clone -b manyuser https://github.com/boji6681/shadowsocksr.git
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/boji6681/doubi/master/service/ssr_centos -O /etc/init.d/ssr; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/boji6681/doubi/master/service/ssr_debian -O /etc/init.d/ssr; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssr
...
Check_Libsodium_ver(){
	...
	Libsodiumr_ver=$(wget -qO- "https://github.com/boji6681/libsodium/tags"|grep "/boji6681/libsodium/releases/tag/"|head -1|sed -r 's/.*tag\/(.+)\">.*/\1/')
...
Install_Libsodium(){
	...
	if [[ ${release} == "centos" ]]; then
		...
		echo -e "${Info} 下载..."
		wget  --no-check-certificate -N "https://github.com/boji6681/libsodium/releases/download/${Libsodiumr_ver}-RELEASE/libsodium-${Libsodiumr_ver}.tar.gz"
		echo -e "${Info} 解压..."
...
# 封禁 BT PT SPAM
BanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/boji6681/doubi/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh banall
	rm -rf ban_iptables.sh
}
# 解封 BT PT SPAM
UnBanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/boji6681/doubi/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh unbanall
	rm -rf ban_iptables.sh
}
Set_config_connect_verbose_info(){
	SSR_installation_status
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/boji6681/doubi/master/ssr.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/ssr" ]]; then
		rm -rf /etc/init.d/ssr
		Service_SSR
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/boji6681/doubi/master/ssr.sh" && chmod +x ssr.sh
...
echo -e "┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
┃  ShadowsocksR 一键管理脚本${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}┃
┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
┃   机场搭建 ❤ 机场托管 ❤ 技术支持 　┃ 
┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
...
━━━━━━━━━━━━
  ...
  ${Green_font_prefix}4.${Font_color_suffix} 无效 libsodium(chacha20)
━━━━━━━━━━━━
