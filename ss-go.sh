...
check_new_ver(){
	new_ver=$(wget -qO- https://api.github.com/repos/boji6681/go-shadowsocks2/releases| grep "tag_name"| head -n 1| awk -F ":" '{print $2}'| sed 's/\"//g;s/,//g;s/ //g')
...
Download(){
	...
	cd "${FOLDER}"
	if [[ ${bit} == "x86_64" ]]; then
		wget --no-check-certificate -N "https://github.com/boji6681/go-shadowsocks2/releases/download/${new_ver}/shadowsocks2-linux.gz"
	else
...
Service(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/boji6681/doubi/master/service/ss_go_centos" -O /etc/init.d/ss-go; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/boji6681/doubi/master/service/ss_go_debian" -O /etc/init.d/ss-go; then
			echo -e "${Error} Shadowsocks 服务管理脚本下载失败 !"
			rm -rf "${FOLDER}"
			exit 1
		fi
...
ss_link_qr(){
	if [[ "${ipv4}" != "IPv4_Error" ]]; then
		...
		SSurl="ss://${SSbase64}"
		SSQRcode="http://xx.xx/qr/qr.php?text=${SSurl}"
		ss_link_ipv4=" 链接  [ipv4] : ${Red_font_prefix}${SSurl}${Font_color_suffix} \n 二维码已失效[ipv4] : ${Red_font_prefix}${SSQRcode}${Font_color_suffix}"
	fi
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/boji6681/doubi/master/ss-go.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/ss-go" ]]; then
		rm -rf /etc/init.d/ss-go
		Service
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/boji6681/doubi/master/ss-go.sh" && chmod +x ss-go.sh
