...
Set_config_user(){
	...
	read -e -p "(默认: Tg@bojiiking):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="Tg@bojiiking"
...
Set_config_port(){
	...
	read -e -p "(默认: 34567):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="34567"
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/boji6681/doubi/master/service/ssrmu_centos -O /etc/init.d/ssrmu; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/boji6681/doubi/master/service/ssrmu_debian -O /etc/init.d/ssrmu; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssrmu
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/boji6681/doubi/master/ssrmu.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	cd "${file}"
	wget -N --no-check-certificate "https://raw.githubusercontent.com/boji6681/doubi/master/ssrmu.sh" && chmod +x ssrmu.sh
