...
check_new_ver(){
	echo -e "只能装 Brook 版本号 v20200801 ${Green_font_prefix}[ 格式是日期，例如: v20200801 ]${Font_color_suffix}
...
Service_brook(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/boji6681/doubi/master/service/brook_centos" -O /etc/init.d/brook; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/boji6681/doubi/master/service/brook_debian" -O /etc/init.d/brook; then
			echo -e "${Error} Brook服务 管理脚本下载失败 !" && rm -rf "${file}" && exit 1
		fi
		chmod +x "/etc/init.d/brook"
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/boji6681/doubi/master/brook.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/brook" ]]; then
		rm -rf /etc/init.d/brook
		Service_brook
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/boji6681/doubi/master/brook.sh" && chmod +x brook.sh
...
if [[ "${action}" == "monitor" ]]; then
	...
	echo && echo -e "┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
┃  Brook 一键管理脚本 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}      ┃
┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
┃   机场搭建 ❤ 机场托管 ❤ 技术支持 　┃ 
┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
